...
All notable changes to `noya-cli` are documented here.
...
- Tracks `noyalib` 0.0.51 under the exact lockstep pin.

## [v0.0.50] - 2026-09-22

### Changed

- Tracks `noyalib` 0.0.50 under the exact lockstep pin.

## [v0.0.49] - 2026-09-22

### Changed

- Tracks `noyalib` 0.0.49 under the exact lockstep pin.
...
- Reworked the README to the ecosystem template while retaining the complete
  command, integration, and verification guide in `docs/README-REFERENCE.md`.
...
- Tracks `noyalib` 0.0.44. The core release fixes a serializer defect
  ...
  else — `noyafmt` writes through noyalib, so this matters here. The fix
  arrives via the version pin; no source change.
...
- this repository's [releases](https://github.com/sebastienrousseau/noya-cli/releases)
  and tags.
