...
#[test]
fn noyavalidate_agrees_with_the_suite_on_every_case() {
    ...
    for case in &cases {
        ...
        let status = std::process::Command::new(env!("CARGO_BIN_EXE_noyavalidate"))
            .arg(&file)
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null())
            .status()
            .expect("run noyavalidate");
        if status.success() == case.fail {
    ...
    eprintln!(
        "yaml-test-suite (noyavalidate): {} passed, {} failed",
